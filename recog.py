...
with dai.Device() as device:
    ...
    while True:
        ...
        classStr = ["Background", "Clover", "Broadleaf", "Grass"]

        # cv2.putText(frame, "NN fps: {:.2f}".format(fps), (2, frame.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 0, 0))
        
        # print(found_classes)
        if len(found_classes) > 1:
            if found_classes[1] == 1:
                cv2.putText(frame, "Found classes " + classStr[1], (2, frame.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 0, 0))
            if found_classes[1] == 2:
                cv2.putText(frame, "Found classes " + classStr[2], (2, frame.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 0, 0))
            if found_classes[1] == 3:
                cv2.putText(frame, "Found classes " + classStr[3], (2, frame.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 0, 0))
        else:
            cv2.putText(frame, "Found classes Background")
